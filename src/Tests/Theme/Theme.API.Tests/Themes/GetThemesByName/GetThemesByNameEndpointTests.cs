...
public class GetThemesByNameEndpointTests : IClassFixture<MartenTestWebApplicationFactory<Program>>
{
    ...
    public GetThemesByNameEndpointTests(MartenTestWebApplicationFactory<Program> factory)
    {
        _factory = factory;
    ...
    [Fact]
    public async Task GetThemesByNameEndpoint_MapGet_WhenCalled_Should_Return_OK_Result()
    {
        //Arrange
        var expectedStatusCode = System.Net.HttpStatusCode.OK;
        var amount = _faker.Random.Int(1, 20);
        var themes = ThemeFakeData.GenerateThemes(amount).ToArray();
        var nameSearch = _faker.Lorem.Letter(1);
        var expected = themes.Where(x => x.Name.ToLower().Contains(nameSearch.ToLower())).Count();
        ...
        //Act
        var response = await _httpClient.GetAsync($"/themes/name/{nameSearch}");
        stopwatch.Stop();

        var responseContent = await JsonSerializer.DeserializeAsync<GetThemesByNameResponse>(await response.Content.ReadAsStreamAsync(), _jsonSerializerOptions);
        ...
        Assert.IsType<GetThemesByNameResponse>(responseContent);

        await _store.Advanced.Clean.DeleteAllDocumentsAsync(); //Clean up after test
    }

    [Fact]
    public async Task GetThemesByNameEndpoint_MapGet_WhenCalled_Should_Return_OK_Empty_Result()
    {
        //Arrange
        var expectedStatusCode = System.Net.HttpStatusCode.OK;
        var nameSearch = _faker.Lorem.Letter(1);
    ...
    public async Task GetThemesByNameEndpoint_MapGet_When_Postgres_Is_Down_Should_Return_Invalid_Operation_Exception_Result()
    {
        ...
        //Act/Assert
        await Assert.ThrowsAsync<InvalidOperationException>(() => _httpClient.GetAsync($"/themes/name/{nameSearch}"));
